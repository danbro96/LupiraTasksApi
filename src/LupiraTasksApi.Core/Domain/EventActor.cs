...
public static class EventActor
{
    ...
    /// <summary>
    /// Stamps actor + actor-email + causation + correlation onto the session before its single commit,
    /// so every event appended in this unit of work carries them. <paramref name="actor"/> is the durable
    /// identity (principal id / <c>share:{label}</c>); <paramref name="actorEmail"/> is a human-audit
    /// convenience header (<c>null</c> for a share write). Causation is the command id; correlation is
    /// the current trace id when a trace is active.
    /// </summary>
    public static void Stamp(IDocumentSession session, string actor, string? actorEmail, Guid commandId)
    {
