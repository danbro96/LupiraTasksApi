...
links, and the internal [`/dav-backend` seam](docs/dav-backend-contract.md) the LupiraDavApi gateway
serves to native task clients as CalDAV (VTODO).
...
REST and MCP are the primary surfaces; share links and the DAV seam are secondary.
...
| Gateway service token | `/dav-backend` | A bearer for this API's audience minted by the DAV gateway's client (`azp` = `DavGateway:ClientId`); the acting user is the `{email}` path segment. |
...
The included [Dockerfile](Dockerfile) is a multi-stage build on the official .NET 10 images,
exposing port `8080`. Build and run:
